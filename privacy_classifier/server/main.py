...
from tester.general_tester import PrivacyClassifierGeneralTester
...
# pipelines = [PrivacyClassifierPipeline() for i in range(3)]
# app.pipelines = InstancePool(pipelines)


@app.post('/predict', dependencies=[Depends(get_token_header)])
def predict(body: PrivacyClassifierInputs, request: Request) -> List[PrivacyClassifierOutputs]:
    ...
    # call model here
    try:
        output: PrivacyClassifierOutputs = pipeline.execute(**data)
...
def parse(body: PrivacyClassifierInputs, request: Request) -> List[PrivacyClassifierOutputs]:
...
model_tester = PrivacyClassifierGeneralTester()
